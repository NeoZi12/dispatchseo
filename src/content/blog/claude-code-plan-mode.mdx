...
`Shift-Tab` cycles a running session through the permission modes - manual, accept-edits, plan, then back to manual - and pressing it twice from manual lands you in plan. Prefixing a single prompt with [`/plan`](/blog/claude-code-slash-commands), or starting the CLI with `claude --permission-mode plan`, does the same thing without the keypresses. Once Claude finishes exploring and drafts a plan, it stops and asks.
...
**Is plan mode the same thing as [`--dangerously-skip-permissions`](/blog/claude-code-bypass-permissions), just inverted?**
