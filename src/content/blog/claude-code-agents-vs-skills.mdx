...
- A **skill** is a directory (or a single file) with [a `SKILL.md` at its root](/blog/how-to-add-skills-to-claude-code): required `name` and `description` frontmatter, an instructions body, and optional bundled scripts or reference files. Claude reads its description at startup and decides on its own when a request matches it.
